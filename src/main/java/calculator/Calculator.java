...
////    }
    ...
    public static double calculatingDiscount(double purchaseAmount, int discountAmount) {
        ...
        double sumDiscount = purchaseAmount - discount;
        if (discountAmount < 0 || purchaseAmount < 0){
            throw new ArithmeticException("Стоимость товара или скидка не может быть ниже нуля");
        }
        if (discountAmount >= 100){
            throw new ArithmeticException("Скидка должна быть меньше 100");
        }
        return sumDiscount; // Метод должен возвращать сумму покупки со скидкой
    }
